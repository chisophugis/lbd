...
/*    Lo = DAG.getNode(ISD::ADD, dl, MVT::i32, HiPart, Lo);
    bool HasGotOfst1 = (GV->hasInternalLinkage() ||
                     (GV->hasLocalLinkage() && !isa<Function>(GV)));
//    if (!HasGotOfst1)
//      return Lo;
    EVT ValTy1 = Op.getValueType();
    GALo = DAG.getTargetGlobalAddress(GV, dl, ValTy1, 0,
                                                        Cpu0II::MO_ABS_LO);
    GALo = DAG.getNode(Cpu0ISD::Lo, dl, ValTy1, GALo);
    return DAG.getNode(ISD::ADD, dl, ValTy1, Lo, GALo);*/
  }

  EVT ValTy = Op.getValueType();
  bool HasGotOfst = (GV->hasInternalLinkage() ||
                     (GV->hasLocalLinkage() && !isa<Function>(GV)));
  int i1 = GV->hasInternalLinkage();
  int i2 = GV->hasLocalLinkage();
  int i3 = isa<Function>(GV);
