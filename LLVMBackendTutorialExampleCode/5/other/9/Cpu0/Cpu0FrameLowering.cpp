...
bool Cpu0FrameLowering::targetHandlesStackFrameRounding() const {
  return true;
}
...
void Cpu0FrameLowering::emitPrologue(MachineFunction &MF) const {
  ...
  const Cpu0RegisterInfo *RegInfo =
    static_cast<const Cpu0RegisterInfo*>(MF.getTarget().getRegisterInfo());
  ...
  // O32.
  if (isPIC && Cpu0FI->globalBaseRegSet() &&
      !Cpu0FI->globalBaseRegFixed()) {
      // See Cpu0InstrInfo.td for explanation.
    ...
    BuildMI(*NewEntry, NewEntry->begin(), dl, TII.get(Cpu0:: SETGP01),
            Cpu0::V0);
  }
  ...
  // ldi sp, sp, (-stacksize)
    BuildMI(MBB, MBBI, dl, TII.get(LDI), SP).addReg(SP).addImm(-StackSize);
...
void Cpu0FrameLowering::emitEpilogue(MachineFunction &MF,
  ...
  // Adjust stack.
  if (isInt<16>(StackSize)) // ldi sp, sp, (stacksize)
    BuildMI(MBB, MBBI, dl, TII.get(LDI), SP).addReg(SP).addImm(StackSize);
  else // Expand immediate that doesn't fit in 16-bit.
	assert("No expandLargeImm(SP, StackSize, false, TII, MBB, MBBI, dl);");
//    expandLargeImm(SP, StackSize, false, TII, MBB, MBBI, dl);
...
// This method is called immediately before PrologEpilogInserter scans the physical registers used to determine what callee saved registers should be spilled. This method is optional. 
