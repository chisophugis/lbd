...
void Cpu0MCInstLower::LowerCPLOAD(SmallVector<MCInst, 4>& MCInsts) {
  ...
  MCSym = MCSymbolRefExpr::Create(Sym, MCSymbolRefExpr::VK_Cpu0_ABS, *Ctx);
  MCOperand SymLo = MCOperand::CreateExpr(MCSym);
  MCInsts.resize(1);
  CreateMCInst(MCInsts[0], Cpu0::LDI, GPReg, SymLo);
}
...
void Cpu0MCInstLower::LowerCPRESTORE(int64_t Offset,
  ...
  if (!isInt<16>(Offset)) {
    ...
    CreateMCInst(MCInsts[0], Cpu0::LDI, ATReg, MCOperand::CreateImm(Hi));
  }
